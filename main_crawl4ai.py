            ...
            messages=[
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "What is the company name shown in this image? Please return only the company name."},
...
async def extract_company_names():
    ...
    async with AsyncWebCrawler(verbose=True, timeout=120000) as crawler:  # 2 minute timeout
        ...
        for page in range(5):
            ...
            try:
                result = await crawler.arun(
                    ...
                    js_code="document.querySelector('.slick-next')?.click();" if page > 0 else None,
            ...
            if not result.success:
                ...
                print(f"[DEBUG] Result error: {getattr(result, 'error', 'No error message')}")
            ...
            for card in cards:
                try:
                    ...
                    if img_result.success and img_result.content:
                        ...
                        img_base64 = base64.b64encode(img_result.content).decode('utf-8')
